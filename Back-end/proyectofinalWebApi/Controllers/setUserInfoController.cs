...
{
    public class setUserInfoController : ApiController
    {
        // GET: api/updateUserInfo
        public async Task<Respuesta> Post(setUserInfoRequest request)
        {
            Respuesta response = await setUserInfo(request.user, request.pass, request.searchedUser, request.userInfoJSON);
            var path = @"C:\xampp\htdocs\proyectofinal2\log.txt";
